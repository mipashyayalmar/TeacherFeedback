...
def ASSIGNMENT(request):
    if request.method == 'POST':
        ...
        return render(request,'authentication/upassign.html',context)
...
def assignments(request):
    ...
    if request.method == 'POST':
        ...
        return render(request,'authentication/assignments.html',context)
...
def class_note(request):
    ...
    if request.method == 'POST':
        ...
        if request.method == 'POST':
          ...
          for m in myfile:
               myclassnote(full_name=fullname,comments=comments,date_field=date_field,file_desc=filedesc,file_name=filename,status=status,myfiles=m).save()
...
def fees(request):
    if request.method == 'POST':
        form =myuploadfees(request.POST,request.FILES)
        ...
        if request.method == 'POST':
          ...
          for j in myfile:
               myuploadfees(roll_no=rollno,f_desc=desc,first_name=firstname,f_name=name,myfiles=j).save()
        ...
        return render(request,'authentication/uploadfees.html',context)
    
def show_fees(request):
    all_data = myuploadfees.objects.all()
